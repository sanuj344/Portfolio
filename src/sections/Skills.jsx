      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {skillsData.map((skillGroup, index) => (
            ...
            >
              <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${skillGroup.gradient} flex items-center justify-center text-3xl mb-6 shadow-md`}>
                {skillGroup.icon}
              </div>
              <h3 className="text-2xl font-bold text-gray-900 mb-6">{skillGroup.category}</h3>
              <div className="space-y-3">
                {skillGroup.skills.map((skill) => (
                  <div key={skill} className="bg-gradient-to-r from-orange-50 to-orange-100 text-orange-700 px-4 py-2 rounded-lg text-sm font-medium border border-orange-200">
                    {skill}
                  </div>
                ))}
              </div>
